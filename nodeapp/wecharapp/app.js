...
server.get("/login", (req, res) => {
  ...
  if (req.session.sid != undefined) {
    pool.query(
      "SELECT uid, uname, phone, img FROM wx_login WHERE uid=?",
      [sid],
      (err, result) => {
        res.send({
          code: 1,
...
server.get("/loginzhuce", (req, res) => {
  ...
  function Ane1() {
    var p = new Promise(() => {
      pool.query(
        "select uname from wx_login where uname=?",
        [uname],
        (err, result) => {
          if (err) console.log(err);
          if (result.length <= 0) {
            setTimeout(() => {
              pool.query(
                "insert into wx_login set uname=?,upass=?,phone=?",
                [uname, upass, phone],
                (err, result) => {
                  if (err) console.log(err);
                  uid = result.insertId;
  ...
  function Ane2() {
    var p = new Promise(function() {
      setTimeout(() => {
        ...
        pool.query(
          "insert into wx_login_chat set login_char=?,istruechat='true',istruenews='true',issearch='true'",
          [uid],
          (err, result) => {
            if (err) console.log(err);
          }
...
server.get("/loginchazhao", (req, res) => {
  ...
  pool.query(
    `select * from wx_login Where uname=?||phone=?||email=?`,
    [uname, phone, email],
    (err, result) => {
      if (err) console.log(err);
      if (result.length <= 0) {
